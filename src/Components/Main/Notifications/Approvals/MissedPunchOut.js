...
const MissedPunchOut = () => {
  ...
  const [punchOutRequest, setPunchOutRequest] = useState([]);
  ...
  const fetchPendingRequests = async () => {
    setLoading(true);
    try {
      const response = await axios.get(`${API_BASE_URL}/api/v1/missedPunchOut/pending-missedPunchOut`, {
        headers: {
      ...
      if (response?.data?.success) {
        setPunchOutRequest(response?.data?.data);
      };
    } catch (error) {
  ...
  const handleStatusChange = async (requestId, newStatus) => {
    try {
      const response = await axios.put(`${API_BASE_URL}/api/v1/missedPunchOut/update-missedPunchOut/${requestId}`,
  ...
  const renderItem = ({ item }) => {
    return (
      <View style={styles.requestItem}>
        ...
        <Text style={styles.date}>Punch Out Time: {formatTimeWithAmPm(item?.punchOutTime)}</Text>
  ...
  return (
    <View style={styles.container}>
      ...
      ) : punchOutRequest?.length === 0 ? (
        <View style={styles.centeredView}>
          <Text style={styles.notFoundText}>No punch out request at the moment.</Text>
        </View>
      ) : (
        <FlatList
